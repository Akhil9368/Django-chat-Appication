...
import threading
import struct
#for Gui i've used tkinter ---> Same thing is done by tkinter that eclipse have done in java chat App
...
class FirstScreen(tk.Tk):
    def __init__(self):
        ...
        background = Image.open("images/login_bg_.jpg")
...
# all the Gui part are here
        tk.Label(self.first_frame, image=background).place(x=0, y=0)

        head = tk.Label(self.first_frame, text="Sign Up / Login", font="lucida 17 bold", bg="sky blue" , height=2)
        head.place(relwidth=1, y=24)

        self.profile_label = tk.Label(self.first_frame, bg="grey")
        self.profile_label.place(x=100, y=135, width=150, height=140)
        ...
        upload_button.place(x=106, y=290)

        self.username = tk.Label(self.first_frame, text="Username", font="lucida 12 bold", bg="white",width=18)
        self.username.place(x=280, y=160)

        self.username_entry = tk.Entry(self.first_frame,  font="lucida 12 bold", width=20,
        ...
        submit_button.place(x=300, y=255)
    ...
    def add_photo(self):
        ...
        if self.image_path:
            ...
            user_image = user_image.resize((120, 120), Image.ANTIALIAS)
    ...
    def process_data(self):
        if self.username_entry.get():
            ...
            try:
                ...
                #exactly same work is done messagedigestbox in java
            ...
            b = b''
            # always true becuase multiple client are come
            while True:
...
# Gui for chat Screen
        ...
        user_image = user_image.resize((30, 30), Image.ANTIALIAS)
        ...
        tk.Label(self, text="   ", font="lucida 15 bold", bg="#cccccc").place(x=4, y=29)
        ...
        # 595656
        # d9d5d4
        container.place(x=40, y=96, width=450, height=550)
        self.canvas = tk.Canvas(container, bg="#dddddd")
        ...
        send_button = tk.Button(self, text="Send", fg="#83eaf7", font="lucida 11 bold", bg="#7d7d7d", padx=10,
                                relief="solid", bd=2, command=self.sent_message_format,width=15,height=1)
        send_button.place(x=320, y=670)

        self.entry = tk.Text(self, font="lucida 10 bold", width=38, height=2,
                             highlightcolor="blue", highlightthickness=1)
        self.entry.place(x=40, y=668)
        ...
        m_label = tk.Label(m_frame, wraplength=250, text=f"Enjoy Chatting {self.parent.user}",
