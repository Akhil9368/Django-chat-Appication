...
#library used for handshaking to server
server_socket.bind(('localhost', 9999))
#accept the handshaking
...
def connection_requests():
    ...
    # connection create and waiting for the client to connect the server
        ...
        if len(clients_connected) == 4:
            ...
            client_socket.close()
            continue
            # data send in the encrypted format
        else:
...
#count the number of clients come
        ...
        #Use the  conecpt of Echo server so the server can send the data all the client that are online
...
# check all the client are online and send the data to all the client
            for client in clients_connected:
                if client != client_socket:
                    client.send('notification'.encode())
                    data = pickle.dumps({'message': f"{clients_connected[client_socket][0]} left the chat",
